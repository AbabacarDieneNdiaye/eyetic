...
import com.ababacardienendiaye.eyetic.config.KafkaProperties;
...
class EyeticKafkaResourceIT {
    ...
    void setup() {
        ...
        EyeticKafkaResource kafkaResource = new EyeticKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/eyetic-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/eyetic-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
